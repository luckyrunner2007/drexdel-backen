...
export const PhoneVerificationScreen: React.FC = () => {
  ...
  const handleSendCode = async () => {
    ...
    setIsSending(true);
    try {
      const res = await drexdelApiClient.sendPhoneVerification();
      if (!res.success) {
        setErrorMessage(res.message || 'Failed to send verification code.');
        return;
      }
      Alert.alert('Code Sent', 'Check your phone for the 6-digit verification code.');
    } catch {
  ...
  const handleVerify = async () => {
    ...
    try {
      const res = await drexdelApiClient.verifyPhone(code);
      if (!res.success) {
  ...
  return (
    <View style={styles.container}>
      ...
      {user?.phoneNumber ? (
        ...
        <Text style={styles.phoneMissing}>No phone number on this account.</Text>
      )}
...
const styles = StyleSheet.create({
  ...
  phone: { fontSize: 16, fontWeight: '700', color: '#7B2CBF', textAlign: 'center', marginBottom: 24 },
  phoneMissing: { fontSize: 14, color: '#6B7280', textAlign: 'center', marginBottom: 24 },
